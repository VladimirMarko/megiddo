...
import { checkFrontendContractBoundaries } from '../scripts/check-frontend-contract-boundaries.mts'
...
test('frontend contract boundary check rejects future UI component contract imports', async () => {
  ...
  try {
    ...
    assert.match(
      result.message,
      /Frontend API Adapter seam rule: frontend UI files must not import contract Resource types directly/,
    )
  } finally {
