...
The root `pnpm dev` command is the supported full local topology. It starts Identity, Todo, API Gateway, and Frontend with the local dummy auth profile for Identity and Todo. `pnpm dev:local` is an alias for the same topology. `pnpm dev:turbo` is a lower-level command for running package dev tasks through Turbo; it is not the authenticated app workflow unless the caller supplies the local auth profile and service URLs.
